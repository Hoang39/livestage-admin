...
import { ReportChatDialog, useReportChatDialog } from "./ReportChatDialog";
...
export const ReportChat = () => {
    ...
    const [dataSource, setDataSource] = useState<ChatReport[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    const [status, setStatus] = useState("A");

    const { openDialog } = useReportChatDialog();

    const fetchData = useCallback(async () => {
        setIsLoading(true);

        try {
            const response = await ReportService.fetchChatReport({
                STATUS: status,
                TARGET_USER_PID: "",
                REPORT_TYPE: "USER",
                PAGE_SIZE: 1000,
            ...
            const dataList = getDataFromPayloadRestful(response).map(
                (item: ChatReport) => ({
                    ...item,
                    REPORT_STATUS: item.WAIT_COMPLAIN_COUNT
                        ? "I"
                        : item.WAIT_APPROVAL_COUNT
                        ? "P"
                        : "",
                })
            );
