...
import {
    ReportBroadcastDialog,
    useReportBroadcastDialog,
} from "./ReportBroadcastDialog";

const { Search } = Input;

export const ReportBroadcast = () => {
    const { t } = useTranslation("Report");
    const { t: tCommon } = useTranslation("Common");

    const [dataSource, setDataSource] = useState<BroadcastReport[]>([]);
    const [isLoading, setIsLoading] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    const [status, setStatus] = useState("R");

    const { openDialog } = useReportBroadcastDialog();

    const fetchData = useCallback(async () => {
        setIsLoading(true);

        try {
            const response = await ReportService.fetchBroadcastsReport({
                KEYWORD: "",
                STATUS: status,
                PAGE_NUM: 1,
                PAGE_SIZE: 1000,
