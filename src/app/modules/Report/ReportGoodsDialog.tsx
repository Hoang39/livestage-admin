...
import { ReportGoodsList } from "./ReportGoodsList";

export const useReportGoodsDialog = dialogStore<GoodsReport>();
...
export const ReportGoodsDialog = ({ onClose = () => "" }: DialogProps) => {
    const { t } = useTranslation("Report");
    const { t: tCommon } = useTranslation("Common");
    const { t: tMembership } = useTranslation("Membership");
    const { openNotification } = useNotificationContext();

    const { open, item, readonly, closeDialog } = useReportGoodsDialog();
            ...
            if (item) {
                if (status == "unban") {
                    await ReportService.removeReport({
                        ...
                        REPORT_PID: item.REPORT_RESULT_PID,
        ...
        <>
            ...
            >
                <Form form={form} layout="vertical" disabled={readonly}>
                    ...
                    {item?.REPORT_STATUS != "P" && (
                        <ReportGoodsList item={item as GoodsReport} />
                    )}
                </Form>
            </Drawer>
        </>
